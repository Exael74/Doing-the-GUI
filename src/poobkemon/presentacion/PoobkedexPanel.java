...
public class PoobkedexPanel extends JPanel {
    ...
    private JPanel createPokemonCard(final String pokemonName) {
        ...
        // Crear BufferedImage estático desde el primer frame del GIF
        try {
            // Cargar el GIF
            ...
            // Crear JLabel con el GIF
            JLabel gifLabel = new JLabel(icon);
            gifLabel.setHorizontalAlignment(JLabel.CENTER);

            // Usar un panel con buffer nativo
            JPanel bufferPanel = new JPanel() {
                // Usar paint en lugar de paintComponent para asegurar limpieza completa
                @Override
            ...
            bufferPanel.add(gifLabel, BorderLayout.CENTER);

            // Añadir al contenedor principal
            container.add(bufferPanel, BorderLayout.CENTER);

        } catch (Exception e) {
            System.err.println("Error al crear card para " + pokemonName + ": " + e.getMessage());
            // Mostrar al menos el nombre si hay error
            JLabel errorLabel = new JLabel(pokemonName);
